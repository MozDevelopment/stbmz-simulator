...
export function AmortizationTable({ amortizationPlan }: AmortizationTableProps) {
    const t = useTranslations('AmortizationTable')
    const totalInterest = amortizationPlan.reduce((sum, entry) => sum + entry.monthlyInterest, 0);
    const totalAmortization = amortizationPlan.reduce((sum, entry) => sum + entry.amortization, 0);
    const totalPayments = amortizationPlan.reduce((sum, entry) => sum + entry.payment, 0);
            ...
            <Table>
                ...
                <TableBody>
                    ...
                    <TableRow>
                        ...
                        <TableCell className="font-bold">{formatCurrency(totalInterest)}</TableCell>
                        <TableCell className="font-bold">{formatCurrency(totalPayments)}</TableCell>
                        <TableCell className="font-bold">{formatCurrency(totalAmortization)}</TableCell>
                    </TableRow>
                </TableBody>
            </Table>
        </div>
